...
type FormTaxProps = PropsWithChildren<DialogProps> & {
  onClose: () => void
  onSave: (message: string, status: string) => void
}

const FormTax: NextPage<FormTaxProps> = ({ open, onClose, onSave }) => {
  const cleanData: ITax = {
    name: '',
    rate: 0,
    type_id: 0
  }
  ...
  const handleSave = () => {
    setIsLoading(true)
    api
      .post('products/taxes/create', customerData)
      .then(({ data }) => {
        handleClose()
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={6}>
                <TextField
                  ...
                  name="rate"
                  type="number"
                  defaultValue={customerData.rate}
                  onChange={handleChange}
                  fullWidth
                  margin="normal"
                  error={Boolean(errors.rate)}
                  helperText={errors.rate == 0 ? '' : errors.rate}
                />
              </Grid>
              <Grid item xs={12}>
                <Autocomplete

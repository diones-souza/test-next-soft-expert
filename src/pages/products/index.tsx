...
const Page: NextPage = () => {
  ...
  const { data, error, isValidating, mutate } = useFetch<IResponse>('products')
  ...
  const handleUpdade = (params: GridCellEditCommitParams) => {
    ...
    const row: IProduct = rows.find(row => row.id === id)

    const updateRow: IProduct = {
      ...row,
      [field]: value
    }

    delete updateRow.id
    delete updateRow.type
    delete updateRow.total_tax
    delete updateRow.created_at

    api
      .put(`products/update/${id}`, updateRow)
      .then(({ data }) => {
        mutate()
  ...
  const handleDelete = async () => {
    ...
    for (const id of selectedRows) {
      await api
        .delete(`products/delete/${id}`, { data: { id } })
        .catch(error => {
          const message = error?.response?.data ?? error?.message

...
const Page: NextPage = () => {
  ...
  const { data, error, isValidating, mutate } = useFetch<IResponse>('users')
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'avatar', headerName: 'Url Avatar', width: 200, editable: true },
  ...
  const handleUpdade = (params: GridCellEditCommitParams) => {
    ...
    const row: IUser = rows.find(row => row.id === id)

    const updateRow: IUser = {
      ...row,
      [field]: value
    }
  ...
  const handleDelete = async () => {
    ...
    for (const id of selectedRows) {
      await api.delete(`users/delete/${id}`, { data: { id } }).catch(error => {
        const message = error?.response?.data ?? error?.message
